...
it('installs php packages', function () {
...
it('installs only require packages', function () {
    $action = new InstallPhpPackagesAction();
    $result = $action->execute(['vendor/package'], []);
    expect($result)->toBeBool();
});

it('installs only require-dev packages', function () {
    $action = new InstallPhpPackagesAction();
    $result = $action->execute([], ['vendor/dev-package']);
    expect($result)->toBeBool();
});

it('handles mixed packages', function () {
    $action = new InstallPhpPackagesAction();
